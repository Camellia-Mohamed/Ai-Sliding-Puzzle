...
def Paly(gameType,gameLevel):
    class Game:
        def __init__(self):
            pygame.init()
            self.amazing=pygame.mixer.Sound('sound/amazing.mp3')
        ...
        def draw_tile(self):
            ...
            for row,x in enumerate(self.tile_grid):
                ...
                for col,tile in enumerate(x):  
                    self.tiles[row].append(Tile(self,col,row,self.gameType,tile,self.tilesize,self.gameSize))
                
        
        def Draw_Button(self):
            # self.screen.fill(backgroud_col)
            self.paly_bfs=Button(width-280,70,150,60,buttons_col[gameType[0]],hover,"BFS",txt_col,25)
            self.paly_bfs.draw_rect(self.screen)
            self.paly_dfs=Button(width-280,150,150,60,buttons_col[gameType[0]],hover,"DFS",txt_col,25)
            self.paly_dfs.draw_rect(self.screen)
            self.paly_ucs=Button(width-280,230,150,60,buttons_col[gameType[0]],hover,"UCS",txt_col,25)
            self.paly_ucs.draw_rect(self.screen)
            self.paly_A=Button(width-280,310,150,60,buttons_col[gameType[0]],hover,"A*",txt_col,25)
            self.paly_A.draw_rect(self.screen)
            self.paly_gready=Button(width-280,390,150,60,buttons_col[gameType[0]],hover,"GREEDY",txt_col,25)
            self.paly_gready.draw_rect(self.screen)
            self.reset=Button(width-295,490,180,60,reset_col,hover,"RESET",reset_txt_col,25)
            self.reset.draw_ellipse(self.screen)
            self.Back = Button(width-70 , 570 , 70 , 30 , back_col , hover , "Back" , back_txt_col , 15)
            self.Back.draw_rect(self.screen)
            
        def steps(self):
            temp = str(self.counter)
            self.count = Button(620 , 10 , 100 , 40 , backgroud_col[gameType[0]] , hover , "STEPS = "+temp , buttons_col[gameType[0]] , 20)
        ...
        def update(self):
            ...
            if self.BFS== True:
                ...
                if self.ind>=len(self.path):
                    channel1.play(self.amazing) 
                    Tile(self,self.gameSize-1,self.gameSize-1,self.gameType,self.gameSize**2,self.tilesize,self.gameSize)
                    self.BFS=False   
...
def level_menu(gameType):
    while True:
        ...
        ChoozeLevel = Button(400 , 130 , 200 , 60 , backgroud_col[gameType[0]], hover , "Choose The Difficulty Level" , buttons_col[gameType[0]] , 40)
        ChoozeLevel.draw_rect(SCREEN)
        easy = Button(200 , 270 , 200 , 60 , buttons_col[gameType[0]] , hover , "EASY" , yellwo , 25)
        easy.draw_rect(SCREEN)
        med = Button(600 , 270 , 200 , 60 ,buttons_col[gameType[0]], hover , "MEDIUM" , yellwo , 25)
        med.draw_rect(SCREEN)
        hard = Button(200 , 410 , 200 , 60 , buttons_col[gameType[0]], hover , "HARD" , yellwo , 25)
        hard.draw_rect(SCREEN)
        ex = Button(600 , 410 , 200 , 60 ,buttons_col[gameType[0]] , hover , "EXTREME" , yellwo , 25)
        ex.draw_rect(SCREEN)
...
def main_menu():
    while True:
        SCREEN.fill(BGColor)
        ChoosePuzzle = Button(400 , 130 , 200 , 60 , BGColor , hover , "Choose The Puzzle" , reset_col , 40)
        ChoosePuzzle.draw_rect(SCREEN)
        IcyTower = Button(100 , 280 , 200 , 60 ,  (132,118,101) , hover , "ICY TOWER" , BGColor , 25)
        IcyTower.draw_rect(SCREEN)
        FeednigFrenzy = Button(360 , 280 , 280 , 60 ,  (132,118,101), hover , "FEEDING FRENZY" ,BGColor , 25)
        FeednigFrenzy.draw_rect(SCREEN)
        FrogFrenzy = Button(700 , 280 , 230 , 60 , (132,118,101), hover , "FROG FRENZY" , BGColor , 25)
        ...
        for event in pygame.event.get():
            ...
            if event.type==pygame.MOUSEBUTTONDOWN: 
                ...
                if IcyTower.click(mouse_x,mouse_y):
                    level_menu(['1'])#gameType should be list 3l4n lma arg3 back a4el a5er 5twa
